...
function getFollowingUsers(req, res) {
    ...
    Follow.find({user: userId}).populate({path: 'followed', model: User}).paginate(page, itemsPerPage, (err, follows, total) => {
...
function getFollowedUser(req,res){
    ...
    Follow.find({followed: userId}).populate({path:'user',model: User}).paginate(page, itemsPerPage, (err, follows, total) => {
        if (err) return res.status(500).send({message: 'server error'});
